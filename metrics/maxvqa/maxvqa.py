...
def check_maxvqa():
    ...
    if not check_docker():
        print("ERROR: Docker is required for MAXVQA but is not available")
        return False

    if not build_docker_image('maxvqa:0.1.0', str(Path(__file__).parent)):
        print("ERROR: Failed to build MAXVQA Docker image")
        return False

    return True


def run_maxvqa(reference, distorted, mode, output_dir=None):
    ...
    if output_dir is not None:
        ...
        temp_fd, output_file = tempfile.mkstemp(suffix='.json', prefix='maxvqa_')
        os.close(temp_fd)
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"ERROR: MAXVQA evaluation failed: {result.stderr}")
            return None
        ...
        print(f"ERROR: MAXVQA evaluation failed: {e}")
        return None
