...
from metrics.utils import get_output_filename, save_json, print_key_value, ts, check_docker, build_docker_image, get_device
...
def check_pyiqa(mode):
    try:
        ...
        print(f"Error checking PyIQA for model {mode}: {e}")
        return False
    return True
    ...
    try:
        ...
        while True:
            ...
            if frame_count % stride == 0:
                ...
                print(f"Frame {processed_count + 1} conversion time: {conversion_time:.4f}s")
                
                # Calculate metric score
                start_inference_time = time.time()
                score = metric(frame_tensor).cpu().numpy()[0]
                inference_time = time.time() - start_inference_time
                print(f"Frame {processed_count + 1} inference time: {inference_time:.4f}s")
...
def run_pyiqa(reference, distorted, mode, output_dir=None):
    ...
    try:
        ...
        print(f"ERROR: PyIQA evaluation failed: {e}")
        return None
